...
from .data.plant_data.compatibility import BirdViewProducerObjectLevelRenderer
...
class ObjectLevelSlotsEncoder(nn.Module):
    def __init__(self, config):
        ...
        #     else:
        #         new_state_dict[k] = v
        self.slot_encoder.load_state_dict(state_dict["state_dict"])
    ...
    def encode(
        ...
        if slots_embeds is None:
            encoded_slots = self.encode_slots(slots) # THESIS: StoSavi call

        else:
            encoded_slots = slots_embeds

        T = slots.shape[1] # Thesis: implementation here we need the real time step amt
        ...
        # encoded_slots = self.slot_projection(encoded_slots) # THESIS: we don't need to do this projection, we just want the slots
